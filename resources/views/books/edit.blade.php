                                            ...
                                            <div class="px-4 py-5 bg-white sm:p-6">
                                                <div class="grid grid-cols-6 gap-6">
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <input type="number" name="isbn" id="isbn" autocomplete="off" value="{{old('isbn') ?? $book->isbn}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                        @error('isbn') <div class="error text-sm text-red-500">{{ $message }}</div> @enderror
                                                    </div>

                                                    <div class="col-span-6 sm:col-span-3">
                                                        <label for="title" class="block text-sm font-medium text-gray-700">Title *</label>
                                                        <input type="text" name="title" id="title" autocomplete="off" value="{{old('title') ?? $book->title}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    ...
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <select id="categories" name="categories[]" autocomplete="off" multiple="multiple" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            @foreach($categories as $key => $category)
                                                                <option value="{{$key}}" {{ in_array($key, $book->categories->pluck('id')->toArray()) ? 'selected' : '' }}>{{$category}}</option>
                                                            @endforeach
                                                    ...
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <select id="authors" name="authors[]" autocomplete="off" multiple class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            @foreach($authors as $key => $author)
                                                                <option value="{{$key}}" {{ in_array($key, $book->authors->pluck('id')->toArray()) ? 'selected' : '' }}>{{ucfirst($author)}}</option>
                                                            @endforeach
                                                    ...
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <input type="number" name="price" id="price" autocomplete="off" step="0.01" value="{{old('price') ?? $book->price}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    ...
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <select id="status" name="status" autocomplete="off" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            <option value="1" {{ ( old('status') == '1' Or $book->status == '1' ) ? 'selected' : '' }}>Available</option>
                                                            <option value="0" {{ ( old('status') == '0' Or $book->status == '0' ) ? 'selected' : '' }}>Unavailable</option>
                                                        </select>
                                                        @error('price') <div class="error text-sm text-red-500">{{ $message }}</div> @enderror
                                                    </div>
                                                </div>
